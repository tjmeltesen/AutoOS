--[[
  AutoOS — Orchestrator raw modem comm test (manager PC)

  No network_protocols / orchestrator FSM — only PING/PONG strings.
  Use this to confirm wireless modems can talk before running broker_main.

  Ports (same as Phase 3):
    listen 105  — this PC receives replies here
    send   106  — messages to the broker go here
...
local LISTEN_PORT = 105
local PEER_PORT = 106
local PEER_ADDR = ""

local ok_cfg, Config = pcall(require, "orchestrator_config")
if ok_cfg and Config then
  PEER_ADDR = Config.broker_address or PEER_ADDR
  LISTEN_PORT = Config.modem_port or LISTEN_PORT
  PEER_PORT = Config.broker_modem_port or PEER_PORT
end
...
local function cmd_info()
  ...
  print("[comm] === orchestrator modem info ===")
...
local function cmd_ping()
  ...
  if not m then print("[comm] " .. err); return end
  if PEER_ADDR == "" then
    print("[comm] broker_address empty in orchestrator_config.lua")
    return
  end
  open_ports(m)
  ...
  if not sent then
    print("[comm] send failed — check broker_address is the broker MODEM uuid")
    return
  end
  ...
  print("[comm] TIMEOUT — no PONG. Is broker running `lua modem_comm_test.lua listen`?")
